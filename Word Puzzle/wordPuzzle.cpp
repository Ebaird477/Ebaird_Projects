...
int main(int argc, char** argv){
    ...
    hashTable prefixes(count);

    ifstream dFile2(dictionary);
    ...
    while(dFile2 >> line){
        ...
        if(line.length() > 3){
            ...
            prefix4Line += line[2];
            prefix4Line += line[3];
            if(!prefixes.find(prefix4Line)){
                prefixes.insert(prefix4Line);
            }
            }
    ...
    bool result = readInGrid(puzzle.c_str(), rows, cols);
    result = true;
    ...
    for(int x = 0; x < rows; x++){
        for(int y = 0; y < cols; y++){
            for(int i = 0; i < 8; i++){
                for(int l = 3; l < 23; l++){
                    string pulledWord = getWordInGrid(x,y,i,l,rows,cols);
                    if(pulledWord.length() < l){
                        break;
                    }
                    if(l >= 4){
                        string testCase = "";
                        testCase += pulledWord[0];
                        testCase += pulledWord[1];
                        testCase += pulledWord[2];
                        testCase += pulledWord[3];
                        bool testCased = prefixes.find(testCase);
                        if(testCased == false){
                            l = 50;
                            break;
                        }
                    ...
                    if(table.find(pulledWord)){
                    wordsFound++;
                    endResult += directions[i] + ": " + "(" + to_string(x) +", "+ to_string(y) + "):         " + pulledWord+ "\n";
                    //cout << directions[i] << ": " << "(" << x <<", "<< y<< "):         " << pulledWord<< endl;
                    }
                    }
...
int timeTaken = int(time * 1000) ;
    // array, in each of the 8 directions
    cout << endl;
    cout << wordsFound << " words found" << endl;
    cout << timeTaken << endl;
    cout << "Found all words in " << t.getTime() << endl;
    cout << timeTaken << endl;
    return 0;
}
